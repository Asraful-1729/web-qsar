    ...
    <SidebarLayout
      ...
      sidebar={
        <aside className="card sticky top-[78px] max-h-[calc(100vh-96px)] overflow-y-auto p-[18px]">
          <SectionIntro title="Target Info" sub="Model metrics & full bucket browser for one target." />
          <PlainTargetSelect targetId={targetId} onChange={setTargetId} />
        </aside>
      }
    >
      <main className="card min-h-[60vh] overflow-hidden">
        {state === "idle" && <EmptyState title="Inspect a target's evidence" hint="Pick a target to see its metrics and files." />}
        ...
        {state === "done" && (
          <div>
            <ResultHeader>
              ...
              <Stat label="Docking">{dockDetail ? "available" : "n/a"}</Stat>
            </ResultHeader>
