...
class VinaEngine(DockingEngine):
    ...
    def dock(self, receptor_profile, ligand_prep, n_poses=9, seed=None):
        """seed: Vina's own --seed. Left unset (Vina picks its own random
           seed) for ordinary compound docking — but redock_reference passes
           a fixed value, since a "standardized" redocking-validation RMSD
           needs the same PDB/ligand/parameters to reproduce the same result
           on every run, not a fresh random search each time (see
           docking/pipeline.py's redock_reference)."""
