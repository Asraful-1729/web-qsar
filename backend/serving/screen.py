...
============================================================
  ...
  caveated shortlist with every signal attached.
    ...
    STEP 6  Docking — if a receptor is available for the target
...
DOCK_WEIGHT = 0.35   # fraction of the fused rank score docking contributes, when
                     # docking actually ran
...
def run(target_id, smiles_list, make_diagram=True, progress=None, advanced=None):
    ...
    if DOCK_PROFILE is not None:
        ...
        # A target with no real co-crystallized ligand (site_source ==
        # "none_validated", see scripts/audit_ligand_selection.py) has no
        # center/box_size at all outside blind mode — basic well-formedness
        # check, not a scientific gate; skip cleanly instead of handing
        # Vina None coordinates once per compound.
        if not blind and dprofile is not None and (not dprofile.get("center") or not dprofile.get("box_size")):
            dprofile = None
            docking_note = (f"Docking skipped: '{target_id}' has no site-specific binding box on record — "
        ...
        if dock_will_run:
            ...
            # above — no separate action. Never fails the shortlist if it
            # errors (informational, not a gate).
            try:
                redocking_validation = DOCK_PIPE.redock_reference_for_profile(
                    dprofile, exhaustiveness=engine.exhaustiveness, n_poses=n_poses)
            except Exception as e:
                redocking_validation = {"status": "error", "error": str(e)}
        elif docking_note is None:
            docking_note = ("Docking skipped: docking engine not available on this machine." if not dock_ready
    ...
    methods_note = (
        ...
        + (f"Docking used AutoDock Vina in BLIND mode ({BLIND_CAVEAT}) and contributed "
           ...
           f"Docking used AutoDock Vina against this target's receptor and contributed "
           f"{int(DOCK_WEIGHT*100)}% of the fused rank score."
           if dock_ran else
    ...
    return {
        ...
        # Redocking-pose validation (see docking/pipeline.py's
        # redock_reference_for_profile) — the target's OWN reference ligand
        # redocked with these SAME settings and compared to its crystal
        # pose. Informational, per-structure, not per-compound.
        "redocking_validation": redocking_validation,
    }
