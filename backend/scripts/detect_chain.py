...
def chain_for_ligand(pdb_path, resname):
    """Returns the (alphabetically first) chain id that contains a HETATM
       ...
       resname may be given as the true CCD identifier (e.g. "A1AWR") even
       if pdb_fetch.py's gemmi fallback had to truncate it to fit legacy
       PDB's 3-character HETATM field — see pdb_fetch.resolve_ligand_resname."""
    from scripts.pdb_fetch import resolve_ligand_resname
    resname = resolve_ligand_resname(pdb_path, resname)
