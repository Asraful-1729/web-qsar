...
from scripts.select_receptor import ADDITIVE_BLACKLIST, MIN_LIGAND_MW, best_ligand_for_entry

PANEL_CSV = os.environ.get("PANEL_RESULTS_CSV", "panel_results_v2.csv")

_ENTRY_RE = re.compile(r"^(\S+)\s+\(res=[^)]*\)\s+ligands:\s*(.*)$", re.DOTALL)
_LIG_RE = re.compile(r"([A-Za-z0-9]{1,5})=[^;]*?\((\d+(?:\.\d+)?)\s*Da\)", re.DOTALL)
