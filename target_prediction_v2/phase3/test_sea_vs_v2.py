"""
Research-handoff item #2, part 2: score real queries with SEA (using the
fitted null distributions from build_sea_null_distributions.py) and
compare against the full assembled v2 pipeline, on a subset of the
fresh-holdout queries (never used in any Phase 3 tuning -- same population
as test_fresh_holdout_validation.py; reusing it for a DIFFERENT comparison
here is fine, only the locked test carries the one-time-only rule).

SEA needs the FULL (uncapped) per-target ligand-set raw score, not the
top-300-neighbour-capped list already stored in the existing capture
files -- so this re-runs the leakage-controlled search per query (same
near-dup + scaffold-strict methodology as every other evaluation this
session), computing the complete Tanimoto vector against the reduced
reference rather than just its top-300.

Subset size (n=150, 75/set) is a disclosed pragmatic choice: the full
leakage-controlled per-query computation costs ~12-13s/query (dominated by
pandas-level near-dup/scaffold masking over millions of rows, not the
Tanimoto math itself) -- same cost profile as every capture script this
session. n=150 keeps this tractable while still giving a real comparison.

Usage: python3 test_sea_vs_v2.py
"""
import json
import os
import random
...
N_PER_SET = 75
SEED = 999  # distinct from every prior sampling seed
DENSITY_THRESHOLD = 8
...
def load_gumbel():
    with open(os.path.join(RESULTS_DIR, "sea_null_distributions.json")) as f:
        d = json.load(f)
    bins = [tuple(b) for b in d["size_bins"]]
    params = d["fitted_gumbel_params"]
    return bins, params


def bin_for_size(size, bins):
    for (lo, hi) in bins:
        if lo <= size < hi:
            return f"{lo}-{hi}"
    return None
...
def main():
    print("Loading v2_index, leak index, gumbel params...", flush=True)
    ...
    size_bins, gumbel_params = load_gumbel()
    ...
    for row in subset:
        ...
        # SEA: full (uncapped) per-target raw score sum. v2_index is already
        # unique per (compound,target) pair, so a plain sum/nunique is
        # correct -- no nested dedup needed.
        ...
        for tcid, rowdata in per_target.iterrows():
            b = bin_for_size(int(rowdata["n_ligands"]), size_bins)
            if b is None or b not in gumbel_params:
                continue
            p = gumbel_params[b]
            pval = float(stats.gumbel_r.sf(rowdata["raw_score"], loc=p["loc"], scale=p["scale"]))
            sea_pvals[tcid] = pval

        sea_ranked = sorted(sea_pvals.items(), key=lambda kv: kv[1])  # ascending p-value = best first

        # v2 pipeline + best_similarity, from the top-300-capped neighbours (already sufficient for k=10 pooling)
        neighbours = []
    ...
    print(f"\n{n_done} queries scored (SEA + v2 + best_similarity)", flush=True)
    ...
    print(f"\nv2 (full pipeline) vs SEA, any-tier:   {ci_v2_vs_sea_any}", flush=True)
    print(f"v2 (full pipeline) vs SEA, primary-tier: {ci_v2_vs_sea_prim}", flush=True)
    print(f"SEA vs v1 best_similarity, any-tier:   {ci_sea_vs_bestsim_any}", flush=True)

    out = {"n": n_done, "v2_vs_sea_any": ci_v2_vs_sea_any, "v2_vs_sea_primary": ci_v2_vs_sea_prim,
           "sea_vs_bestsim_any": ci_sea_vs_bestsim_any}
    with open(os.path.join(RESULTS_DIR, "sea_vs_v2_result.json"), "w") as f:
        json.dump(out, f, indent=2, default=str)
    print(f"\nWrote {os.path.join(RESULTS_DIR, 'sea_vs_v2_result.json')}", flush=True)
