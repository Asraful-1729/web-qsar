...
A CPU-only tool for prioritising compounds against a validated biological
target — QSAR potency (Chemprop + AutoGluon), ADMET profiling, and
structure-based docking. Two independent services: a FastAPI JSON API
(`backend/`) and a React single-page UI (`frontend/`), talking to each
other over HTTP/CORS.

This is the **web-only** copy of PhytoScreen (extracted from the
`qsar-desktop` project, which also builds a native Windows desktop app via
PyInstaller + Inno Setup). Nothing desktop-specific was copied here:
`desktop.py`, the Windows `installer/`, the Windows CI workflow, the
bundled Windows `bin/vina.exe`, and the `pywebview` dependency all stay in
`qsar-desktop` — this tree only has what's needed to run `backend/` +
`frontend/` as ordinary web services. Install real AutoDock Vina for your
platform (`conda install -c bioconda autodock-vina`, or your package
manager) so it resolves on `$PATH` — `docking/engines.py` looks it up by
name (`shutil.which("vina")`), it doesn't expect a bundled binary.
...
## Layout
    ...
    models/curated/          bundled decoy-generation source data (~15MB, committed)
    models/<target_id>/      per-target buckets — NOT bundled; downloaded on demand
                             from R2 by backend/downloads.py the same way the desktop
                             app does (read-only once fetched)
    docking_targets/<id>/    prepared receptors — same on-demand download, not bundled
